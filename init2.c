...
int sphyraena_init(sphyraena *s, sqlite3 *db, size_t data_size, int pinned_memory)
{
	...
	if(pinned_memory) {
		...
		// allocate memory for data on the cpu side
		...
		if(s->data_cpu == NULL || s->stmt_cpu == NULL) {
			fprintf(stderr, "Mallocs failed");
			return SPHYRAENA_ERR_MEM;
		}
	}

	// allocate memory for data on the gpu side
